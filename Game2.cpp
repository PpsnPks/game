...
/* กรอบเลือกความยาก */void draw_frame_level(int line) {
    gotoxy(35, line); printf("**********");
    gotoxy(35, ++line); printf("*");
    gotoxy(44, line); printf("*");
    gotoxy(35, ++line); printf("**********");
}
    ...
    do {
        if (_kbhit()) {
            ...
            if (ch == 'w') {
                ...
                if (line > 1)
                    draw_frame_level(line -= 4), hard = line;
                else line = 17, draw_frame_level(line), hard = line;
            }
            fflush(stdin);
        }
        Sleep(100);
...
void draw_status(){
    ...
    gotoxy(32 + ((p.Hard - 1) * 8), 20); printf(" 4.Random Box       %d  ", p.Storage[3]);
    setcolor(7, 0);
}
...
int play_Game() {
    ...
    clear_screen();
    start();
    if (p.Hard == 1)setConsole(57, 21);
    if (p.Hard == 2)setConsole(67, 25);
    if (p.Hard == 3)setConsole(72, 28);
    if (p.Hard == 4)setConsole(84, 33);
